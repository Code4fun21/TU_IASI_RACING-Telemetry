        ...
        try {
            ...
            for (const sig of signals) {
                ...
                if (sig.method === "Float32_LE") {
                    // Pull standard Little Endian float
                    finalVal = buffer.getFloat32(sig.offset, true); 
                    rawVal = finalVal; // Sync raw for multipliers below
                } else {
                    // Standard DataView reading natively handles standard Big Endian 
                    rawVal = this.readBytes(buffer, sig.offset, sig.size, sig.isSigned);
                    finalVal = rawVal;
                }

                // 2. Apply Custom Methods (if not standard Float32)
                if (sig.method && sig.method !== "Float32_LE") {
                    ...
                    // 3. Apply Multipliers and Dividers (Now supports Float32_LE like GPS_Speed)
            ...
            // IMU Rotation Logic - Triggered exactly by your CAN ID 0501
            if (canId === "0501") {
                this.lastAccel = [
            ...
            if (decodedValues["GPS_Latitude"] && decodedValues["GPS_Longitude"]) {
                ...
                // Fallback to last known speed since speed arrives on a different CAN ID (0801)
                ...
                // If we receive the 0801 Speed message on its own, apply it to current position
